...
@login_required
@user_passes_test(lambda u: u.is_superuser)
def update_about_order(request, about_id):
    about = get_object_or_404(About, id=about_id)
    if request.method == 'POST':
        new_order = request.POST.get('order')
        if new_order is not None:
            try:
                about.order = int(new_order)
                about.save()
                messages.success(request, 'About page order updated successfully.')
            except ValueError:
                messages.error(request, 'Invalid order value.')
        else:
            messages.error(request, 'Order value is required.')
...
@login_required
@user_passes_test(lambda u: u.is_superuser)
def bulk_contact_actions(request):
    if request.method == 'POST':
        selected_contacts = request.POST.getlist('selected_contacts')
        action = request.POST.get('action')
        
        if action == 'mark_read':
            ContactSubmission.objects.filter(id__in=selected_contacts).update(is_read=True)
            messages.success(request, f'{len(selected_contacts)} contacts marked as read.')
        elif action == 'delete':
            ContactSubmission.objects.filter(id__in=selected_contacts).delete()
            messages.success(request, f'{len(selected_contacts)} contacts deleted.')
